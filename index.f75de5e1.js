...
titlesList.forEach((title, index)=>{
    ...
    let inputType = "text";
    if (index === 3 || index === 4) inputType = "number";
    if (index !== 2) {
        const input = document.createElement("input");
        input.setAttribute("name", qaAtr);
        input.setAttribute("type", inputType);
        input.setAttribute("data-qa", qaAtr);
        if (index === 0 || index === 1) input.setAttribute("required", "");
...
function sortTable(columnIndex, dataType, sortOrder) {
    rows.sort((rowA, rowB)=>{
        ...
        if (dataType === "number") return sortOrder === "ASC" ? helper(cellA) - helper(cellB) : helper(cellB) - helper(cellA);
        else return sortOrder === "ASC" ? cellA.localeCompare(cellB) : cellB.localeCompare(cellA);
    });
}
button.onclick = (e)=>{
    e.preventDefault();
    ...
    Object.values(formData).forEach((data, index)=>{
        ...
        let v = data;
        if (index === 3) v = Number(data);
        if (index === 4) v = `$${Number(data).toLocaleString("en-US")}`;
